...
import { isValidTriplePanna } from '../utils/pannaValidation';

export default function TriplePannaScreen() {
  ...
  const handleAddBid = async () => {
    ...
    if (!panna.trim()) {
      Alert.alert('Error', 'Please enter a triple panna');
      return;
    }
    if (!amount.trim()) {
    ...
    if (panna.length !== 3) {
      Alert.alert('Error', 'Please enter exactly 3 digits for triple panna');
      return;
    }
    
    // Validate panna using the valid array
    if (!isValidTriplePanna(panna)) {
      Alert.alert('Error', 'Please enter a valid triple panna number');
      return;
    }
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Triple Panna ({gameName})</Text>
      ...
      <View style={styles.inputSection}>
        <View style={styles.inputContainer}>
          <TextInput
            ...
            placeholder={hasBalance ? "Enter Triple Panna (000-999)" : "Add funds to place bid"}
            value={panna}
          ...
          {panna.length === 3 && !isValidTriplePanna(panna) && (
            <Text style={styles.errorText}>Invalid triple panna number</Text>
          )}
        </View>
      ...
      <TouchableOpacity 
        style={[
          styles.addBidButton, 
          (isLoading || !hasBalance || (panna.length === 3 && !isValidTriplePanna(panna))) && styles.disabledButton
        ]} 
        onPress={handleAddBid}
        disabled={isLoading || !hasBalance || (panna.length === 3 && !isValidTriplePanna(panna))}
      >
        {isLoading ? (
